---
helpx_url: "https://helpx.adobe.com/substance-3d-designer/substance-compositing-graphs/nodes-reference-for-substance-compositing-graphs/node-library/material-filters/blending-material.html"
breadcrumb-title: ''
description: Accedete ai nodi di fusione dei materiali in Substance 3D Designer per combinare e fondere i materiali per creare effetti di materiale composito.
helpx_creative_field: ""
helpx_description: Designer > Substance compositing graphs > Nodes reference for Substance compositing graphs > Node library > Material Filters > Blending (Material)
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: Fusione (materiale)
user-guide-description: ''
user-guide-title: ''
source-git-commit: 6c55ac0f1f6da5bc5683a34a4eca174f978eac64
workflow-type: tm+mt
source-wordcount: '87'
ht-degree: 0%

---


# Fusione (materiale)

La categoria Fusione è una delle più importanti categorie di materiale multicanale. Contiene nodi destinati a combinare, commutare e miscelare materiali che consistono in molteplici output simultanei.

Questi nodi consentono di risparmiare una grande quantità di lavoro, ma è importante comprendere appieno le [modalità di creazione dei collegamenti](../../../../../interface/the-graph-view/link-creation-modes/link-creation-modes.md) per utilizzarle correttamente.

I nodi seguenti sono quelli utili per iniziare:

* [Fusione multimateriale](../../../../../compositing-graphs/nodes-reference-for-com/node-library/material-filters/blending-material/multi-material-blend/multi-material-blend.md)
* [Fusione di regolazione materiale](../../../../../compositing-graphs/nodes-reference-for-com/node-library/material-filters/blending-material/material-adjustment-blend/material-adjustment-blend.md)
