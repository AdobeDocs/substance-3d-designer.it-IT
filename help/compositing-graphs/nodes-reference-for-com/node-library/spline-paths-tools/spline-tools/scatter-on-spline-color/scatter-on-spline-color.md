...
<b>Inizio</b> *Mobile* Sposta il punto dall&#39;inizio di una spline nel punto in cui inizia la dispersione. Il valore è la lunghezza normalizzata di ogni spline.
...
* X: il Thickness viene moltiplicato per la dimensione solo sull’asse X;
...
* *Fusione Alpha*: applica una semplice fusione di trasparenza utilizzando il canale alfa del pattern. I motivi disegnati per ultimi sono in primo piano.

<b>Modalità colore</b> *Intero* Metodo di fusione con cui si seleziona il colore di ogni pattern:\
*- Colore di base*: il colore di base viene applicato a tutti i pattern;
* *Posizione*: la posizione del pattern nello spazio della texture viene utilizzata per guidarne il colore in modo che le coordinate X e Y siano mappate rispettivamente ai canali del rosso e del verde.

<b>Colore base forma</b> *Float4* Colore di base dei pattern.

<b>Moltiplicatore input colore</b> *Mobile* Controlla l’intensità dell’input Mappa colori. Questa mappa funge da moltiplicatore per il colore corrente dei pattern.\
...
<b>Modalità campionamento input mappa colori</b> *Intero* Metodo di mappatura dei valori nella mappa colori sulle spline:\
*- Spazio texture*: i valori vengono applicati alle spline in cui si troverebbero se fossero inseriti in una texture utilizzando le coordinate UV della texture. In questo modo si applica effettivamente il valore alle spline &quot;in posizione&quot;;\
*- Orizzontale lungo la spline*: i valori vengono applicati direttamente alle coordinate delle spline codificate (vedere l&#39;input Coord spline), in cui ogni riga viene applicata a una spline diversa dall&#39;alto verso il basso;\
*- Ora. lungo spline (rand. offset X)*: i valori vengono applicati direttamente alle coordinate delle spline codificate (vedere l&#39;input Coords spline), con uno scostamento orizzontale casuale nella mappa colore per ogni spline (ovvero, ogni riga nelle coord spline);\
*- Ora. lungo spline (rand. offset Y)*: i valori vengono applicati direttamente alle coordinate delle spline codificate (vedere l&#39;input Coord spline), con uno scostamento verticale casuale nella mappa colore per ogni spline (ovvero, ogni riga nelle coordinate spline).

<b>Colore casuale</b> *Float4* Applica uno scostamento casuale fino ai valori specificati ai colori dei pattern nello spazio HSV, nonché al relativo canale alfa.\
*Nota:* il colore di output è il risultato ponderato di tutti i moltiplicatori di colore.

<b>Centro colore casuale</b> *Mobile* Applica uno scostamento all&#39;intervallo dello scostamento casuale applicato in Colore casuale\
Un valore pari a -1 significa che tutti i valori casuali sono più alti e un valore pari a 1 significa che tutti i valori casuali sono più bassi.
...
<b>Moltiplicatore Thickness spline</b> *Fluttuazione* Intensità per la quale l&#39;alfa di ogni pattern viene moltiplicato per il thickness della spline nella relativa posizione.\
Nota: il colore di output è il risultato ponderato di tutti i moltiplicatori di colore.

<b>Moltiplicatore scala forme</b> *Fluttuazione* Intensità di cui l&#39;alfa di ogni pattern viene moltiplicato rispetto alla scala.\
Nota: il colore di output è il risultato ponderato di tutti i moltiplicatori di colore.

<b>Moltiplicatore Indice Forma</b> *Fluttuazione* Intensità per la quale l&#39;alfa di ogni pattern viene moltiplicato rispetto all&#39;indice normalizzato.\
Nota: il colore di output è il risultato ponderato di tutti i moltiplicatori di colore.

<b>Moltiplicatore Height spline</b> *Fluttuazione* Intensità per la quale l&#39;alfa di ogni pattern viene moltiplicato per il height della spline nella relativa posizione.\
Nota: il colore di output è il risultato ponderato di tutti i moltiplicatori di colore.

<b>Luminanza casuale</b> *Mobile* Applica un moltiplicatore casuale fino al valore specificato per diminuire il valore alfa dei pattern.\
Nota: il colore di output è il risultato ponderato di tutti i moltiplicatori di colore.
