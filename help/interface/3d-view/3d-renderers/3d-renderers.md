...
| **Attiva antialiasing accumulo rasterizzatore** booleano | Attiva/disattiva l’antialiasing di accumulo, che migliora lo smoothness o i bordi dell’immagine renderizzata modificando i rendering e calcolando cumulativamente il colore medio locale di ciascun pixel. Cioè, accumula valori da cui calcolare una media. |
...
| **Height** Mobile | Controlla lo scostamento height del piano di massa.   Se viene creato, il valore dovrebbe avere la distorsione appropriata, in base alla scala della scena. |
| **Intensità ombra** Mobile | Quando le ombre sono attivate, controlla l’opacità delle ombre proiettate sul piano terreno, da 0,0 (nessuna ombra) a 1,0 (ombra intera). |
...
| **Opacità riflessi** float | Quando le riflessioni sono attivate, controlla l’opacità delle riflessioni compresa tra 0,0 (nessuna riflessione) e 1,0 (riflessione completa). |
| **Rugosità riflessi** Mobile | Quando le riflessioni sono attivate, controlla la rugosità del materiale del piano di massa che contribuisce alle riflessioni, da 0,0 (completamente lucido) a 1,0 (completamente irregolare). |
