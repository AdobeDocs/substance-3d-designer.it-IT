...
* **La gestione del colore viene applicata in tre modi diversi**\
  ...
  * **Input e output grafici ed esportazione bitmap**\
    Quando si leggono e scrivono immagini bitmap all’ingresso e all’uscita di un grafico, la gestione del colore può regolare l’immagine e convertirla per funzionare nelle condizioni corrette. Questo può essere controllato nella preferenza principale per bitmap nel grafico e modificato nella finestra di dialogo di esportazione durante il processo di esportazione. Il salvataggio di un’immagine dal vista 2D consente anche di scegliere lo spazio colore.
...
### Nuova curvatura da Baker trama

![](version-2019-3-9-3.resources/banner-curvature-1.png)

Il baker Curvatura da trama è stato rifatto da zero. Ora sfrutta il raytracing, che consente l’accelerazione GPU, ma aggiunge anche il supporto delle intersezioni di trama.
...
>[!NOTE]
>
> La precedente Curvatura da baker trama è ora obsoleta. Si consiglia di passare alla nuova versione.

### Occlusione ambientale migliorata dal Baker trama

![](version-2019-3-9-3.resources/banner-ao-v2.png)

L’Occlusione ambientale Da baker trama ora ha una nuova impostazione denominata &quot;Piano terreno&quot; che simula un piano sotto la trama per produrre un’occlusione di terreno. Questo può aiutare a produrre risultati più interessanti per la geometria statica o gli oggetti non organici. Per impostazione predefinita, il piano terreno si trova nella parte inferiore del rettangolo di selezione della trama, ma può essere spostato ulteriormente con impostazioni dedicate.

Per ulteriori informazioni, consulta la documentazione [Occlusione ambientale da Mesh](https://experienceleague.adobe.com/it/docs/substance-3d/bakers/bakers-settings/ambient-occlusion-from-mesh).
...
* **Visibile se sta prendendo in considerazione i gizmo di Trasforma 2D**\
  Quando un gizmo Trasforma 2D ha una condizione If visibile, può ora essere nascosto nel vista 2D, rendendolo contestuale ai parametri.
