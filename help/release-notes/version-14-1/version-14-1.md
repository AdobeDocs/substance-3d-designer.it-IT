...
Questo aggiornamento introduce nuove funzioni per migliorare l’utilizzo quotidiano di Substance 3D Designer: strumenti di disposizione dei nodi per migliorare rapidamente il layout del grafico, parametri di copia/incolla per applicare un set di parametri a un altro nodo e una puntina pixel nella vista 2D per tracciare un pixel specifico durante il debug del grafico. Viene inoltre aggiunto nuovo contenuto, principalmente per completare i set di nodi Spline e Path.
